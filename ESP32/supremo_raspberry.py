...
def calibrate_color(event, x, y, flags, param):
    ...
    if event == cv.EVENT_LBUTTONDOWN:
        color = frame[y, x]
        lower_bound = np.array([color[0] - 40, color[1] - 40, color[2] - 100])
        upper_bound = np.array([color[0] + 40, color[1] + 40, color[2] + 100])
        calibrated = True
